...
EN = {"Cancel": "Cancel", "Bold": "Bold", "Italic": "Italic", "Underline": "Underline", "Overstrike": "Strikethrough"}
FR = {"Cancel": "Annuler", "Bold": "Gras", "Italic": "Italique", "Underline": "Souligné", "Overstrike": "Barré"}
...
class FontChooser(Toplevel):
    ...
    def __init__(self, master, font_dict={}, text="Abcd", **kwargs):
        ...
        self.var_family = StringVar(self)
        self.var_size = StringVar(self)
        entry_family = Entry(self, width=max_length, validate="key",
                              textvariable=self.var_family,
                              validatecommand=(self._validate_family, "%P", "%V"))
        ...
        if len(text) > 30:
            text = text[:30]
        self.preview = Label(self, relief="groove", text=text, style="prev.TLabel",
                             font=self.preview_font, anchor="center")

        # Widget configuration
        self.list_family.configure(yscrollcommand=scroll_family.set)
        self.list_size.configure(yscrollcommand=scroll_size.set)

        entry_family.insert(0, font_dict["family"])
        entry_size.insert(0, font_dict["size"])

        i = self.fonts.index(self.var_family.get().replace(" ", "\ "))
        ...
        entry_family.grid(row=0, column=0, sticky="ew", pady=(10,1), padx=(10,0))
        entry_size.grid(row=0, column=2, sticky="ew", pady=(10,1), padx=(10,0))
        ...
        entry_family.bind("<Return>", self.change_font_family)
...
def askfont(master=None, text="Abcd", **font_args):
    """ Open the font chooser and return the chosen font.
        ...
        font_args: family, size, slant (=roman/italic), weight (=normal/bold), underline (bool), overstrike (bool)"""
